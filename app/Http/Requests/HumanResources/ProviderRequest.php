...
//		dd(date('d/m/Y'));
		...
		switch ( $this->method() ) {
			...
			case 'PATCH':
				{
					$Provider = Provider::findOrFail($this->provider);

					if ( $this->get( 'type' ) ) {
						$rules['cnpj'] = 'required|min:3|max:20|unique:legal_people,cnpj,' . $Provider->legal_person_id . ',id';
						$rules['ie']   = ( $this->get( 'exemption_ie' ) == 1 ) ? '' : 'required|min:3|max:20|unique:legal_people,ie,' . $Provider->legal_person_id . ',id';
						//juridica

					} else {
						//fisica
						$rules['cpf'] = 'required|min:3|max:20|unique:' . $this->table . ',cpf,' . $this->provider . ',id';
					}
