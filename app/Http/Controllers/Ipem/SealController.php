...
use App\Filters\SealFilter;
use App\Http\Controllers\Controller;
use App\Http\Requests\Ipem\SealRequest;
...
class SealController extends Controller {
	...
	public $name = "Lacre";
	public $names = "Lacres";
	public $main_folder = 'pages.ipem.seals';
	public $page = [];
	public $SealFilter;
	...
	public function __construct( Route $route ) {
		...
		$this->SealFilter = new SealFilter();
	}
	...
	public function index(IlluminateRequest $request)	{
		$labels = Seal::query(); //$this->user->labels();
		$this->page->response = $this->SealFilter->map($request, $labels);
	...
	public function create( ) {
		...
		return view('pages.ipem.seals.create' )
			->with( 'Page', $this->page );
	}
	/**
	 * Store the specified resource in storage.
	 *
	 * @param  \App\Http\Requests\Ipem\SealRequest $request
	 *
	 * @return \Illuminate\Http\Response
	 */
	public function store( SealRequest $request ) {
		...
		if (Seal::whereIn('number', $values)->count() > 0) {
			return $this->error( ['Já existem Lacres com essa numeração!'] );
		} else {
			...
			Seal::insert($data);
			$qtd = count($data);
			return response()->success( 'Foram adicionadas ' . $qtd . ' Lacres: ' . implode('; ', $values), $data, route( 'seals.create' ) );
		}
	}
	...
	public function tracking( ) {
		...
		$requests = Request::seals(); //$this->user->labels();
		...
		return view('pages.ipem.seals.requests' )
			->with( 'Page', $this->page );
	}
	...
	public function deny(IlluminateRequest $request) {
		...
		return response()->success( 'Requisição negada com sucesso!', $data, route( 'seals.tracking' ) );
	}
	...
	public function accept(IlluminateRequest $request) {
		...
		return response()->success( 'Requisição aceita com sucesso!', $data, route( 'seals.tracking' ) );
	}
