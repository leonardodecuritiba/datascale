...
class LabelFilter
{
	...
	public function filter(Request $request)
	{
		...
		if(
			...
			$query = DB::table( 'label_instruments' )
			           ->join( 'apparatus', function ( $join ) {
				           $join->on( 'apparatus.id', '=', 'label_instruments.apparatu_set_id' )
				                ->orOn( 'apparatus.id', '=', 'label_instruments.apparatu_unset_id' );
			           });
			...
			if($order_id!='' || $cnpj!='') {
				...
				if($cnpj!='') {
					$query->join( 'clients', 'clients.id', '=', 'orders.client_id' )
					      ...
					      ->select( 'label_instruments.label_id' )
			...
			$filter->whereIn( 'id', $query->pluck( 'label_id' ) );
		}
		return $filter;
	}
		...
		if($request->has('search') || $request->has('search_id')){
			...
			if(!is_a($filter, 'Illuminate\Database\Eloquent\Collection')) {
				$filter = $filter->get()->map( function ( $s ) {
					...
					$label_instrument = $s->label_instrument;
					if($label_instrument!=NULL){
						$instrument         = $label_instrument->instrument;
						$client             = $instrument->client;
						$order_setted_id    = ($label_instrument->apparatu_set_id != NULL) ? $label_instrument->apparatu_set->order_id : NULL;
						$order_unsetted_id  = ($label_instrument->apparatu_unset_id != NULL) ? $label_instrument->apparatu_unset->order_id : NULL;
